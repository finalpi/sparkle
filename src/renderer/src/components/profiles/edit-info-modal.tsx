              ...
              <Surface variant="transparent" className="flex flex-col">
                ...
                {values.type === 'remote' &&
                  ...
                  renderField(
                    ...
                    <Switch
                      ...
                      onChange={(locked) => setValues({ ...values, locked })}
                    >
                      <Switch.Content>
                        <Switch.Control>
                          <Switch.Thumb />
                    ...
                    </Switch>,
                    {
                      actions: (
                        <Tooltip delay={0}>
                          ...
                          <Tooltip.Content>远程订阅提供更新间隔时默认锁定，可关闭后自行设置</Tooltip.Content>
                        </Tooltip>
                      )
                    }
                  )}
                {values.type === 'remote' &&
                  ...
                  renderField(
                    ...
                    <select
                      aria-label="更新方式"
                      value={values.updateSchedule?.type ?? 'interval'}
                      disabled={values.locked}
                      className="h-9 w-full rounded-md border border-default-200 bg-default-100 px-3 text-sm outline-none disabled:opacity-50"
                      onChange={(event) => {
                        const type = event.target.value as 'interval' | 'daily' | 'weekly'
                        setValues({
                          ...values,
                          updateSchedule: {
                            type,
                ...
                {values.type === 'remote' &&
                  values.autoUpdate &&
                  (values.updateSchedule?.type ?? 'interval') === 'interval' &&
                  renderField(
                    '更新间隔（分钟）',
                    <Input
                      aria-label="更新间隔（分钟）"
                ...
                {values.type === 'remote' &&
                  ...
                  renderField(
                    ...
                    <select
                      aria-label="星期几"
                      value={values.updateSchedule.weekday ?? 0}
                      disabled={values.locked}
                      className="h-9 w-full rounded-md border border-default-200 bg-default-100 px-3 text-sm outline-none disabled:opacity-50"
                      onChange={(event) => {
                        setValues({
